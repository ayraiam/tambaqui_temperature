...
if [[ -z "${ENRICH_DESEQ_TSV}" ]]; then
  if [[ -n "${DESEQ2_CONTRAST_VARIABLE}" && -n "${DESEQ2_CONTRAST_NUMERATOR}" && -n "${DESEQ2_CONTRAST_DENOMINATOR}" ]]; then
    ENRICH_DESEQ_TSV="${DESEQ2_OUTDIR}/${DESEQ2_CONTRAST_VARIABLE}_${DESEQ2_CONTRAST_NUMERATOR}_vs_${DESEQ2_CONTRAST_DENOMINATOR}_all_genes.tsv"
  fi
fi
